...
Node LoadNone() { return Node(); }

Node LoadBool(bool state) { return Node(state); }

Node LoadNull() { return Node(); }
...
Node::Node(nullptr_t n) : value_(n) {}

Node::Node(Array array_) : value_(move(array_)) {}
...
bool Node::IsString() const { return holds_alternative<std::string>(value_); }
...
void PrintValue(int dig, std::ostream& out) { out << dig; }
void PrintValue(double dig, std::ostream& out) { out << dig; }
