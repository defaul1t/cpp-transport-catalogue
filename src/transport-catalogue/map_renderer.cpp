...
void MapRenderer::CreateBusNames() {
  ...
  for (auto& name : name_bus) {
    if (ts_->FindBus(name)->stops_.empty()) {
      continue;
    }
    ...
    sub_text.SetPosition(
        sph->operator()(ts_->FindBus(name)->stops_.at(0)->coord_));
    ...
    if (!ts_->FindBus(name)->is_roundtrip_ &&
        ts_->FindBus(name)->stops_.at(ts_->FindBus(name)->stops_.size() / 2) !=
            ts_->FindBus(name)->stops_.at(0)) {
      text.SetPosition(sph->operator()(
          ts_->FindBus(name)
              ->stops_.at(ts_->FindBus(name)->stops_.size() / 2)
              ->coord_));
      sub_text.SetPosition(sph->operator()(
          ts_->FindBus(name)
              ->stops_.at(ts_->FindBus(name)->stops_.size() / 2)
              ->coord_));
      scheme_.Add(sub_text);
...
void MapRenderer::CreateStopCircles() {
  ...
  for (auto& name : stopname) {
    if (ts_->FindStop(name)->buses_.empty()) {
      continue;
    }
...
void MapRenderer::CreateStopNames() {
  for (auto& name : stopname) {
    ...
    text.SetOffset(
        svg::Point(stop_label_offset.at(0), stop_label_offset.at(1)));
    sub_text.SetOffset(
        svg::Point(stop_label_offset.at(0), stop_label_offset.at(1)));
...
void MapRenderer::CreateSvg(std::ostream& os) {
  ...
  if (!coord.empty()) {
    SphereProjector sph_iden{coord.begin(), coord.end(), widght, height,
                             padding};
    sph = &sph_iden;
...
void MapRenderer::SetTransportCatalogue(
