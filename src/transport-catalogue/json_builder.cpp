  ...
  if (dict_active) {
    if (!key_active) {
      throw std::logic_error(
          "Wrong StartDict() Flag(without key in dict added)");
    }
    key_active = false;
    ...
    throw std::logic_error("Wrong StartDict() Flag");
  }
  dict_active = true;
...
Builder::ArrayValueContext Builder::StartArray() {
  if (dict_active) {
    if (!key_active) {
      throw std::logic_error(
          "Wrong StartArray() Flag(without key in dict added)");
    }
    key_active = false;
    node_stack_.push_back(new Node(std::move(key_)));
    node_stack_.push_back(new Node(std::move(dict_)));
    ...
    throw std::logic_error("Wrong StartArray() Flag");
  }
  array_active = true;
