 ...
 */
std::vector<std::string> ParserByParameters(std::string_view string_line,
                                            char type_delimeter) {
  std::vector<std::string> res;

  res.reserve(
      std::count(string_line.begin(), string_line.end(), type_delimeter));
  std::string parameter;
  for (auto symbol : string_line) {
    if (symbol == type_delimeter) {
      parameter.erase(parameter.begin(),
                      parameter.begin() + parameter.find_first_not_of(" "));
      parameter.erase(parameter.begin() + parameter.find_last_not_of(" ") + 1,
                      parameter.end());
...
void inreader::InputReader::ParseLine(std::string_view line) {
  ...
  if (line.find(":") != std::string::npos) {
    if (line[0] == 'B') {
      ...
      res.description =
          std::string(line.begin() + line.find_first_of(":") + 1, line.end());
 ...
 */
void inreader::InputReader::ApplyCommands(
    transport_catalogue::processing::TransportCatalogue& catalogue,
    bool first_proccess, std::vector<std::string>&& save_) {
  if (first_proccess) {
    ...
    if (command.command == "AStop") {
      if (first_proccess) {
        ...
        std::vector<std::string> params =
            std::move(ParserByParameters(command.description, ','));
        ...
        std::vector<std::string> params =
            std::move(ParserByParameters(command.description, ','));
        if (params.size() >
            2)  // первые два это координата, остальные строки с дистанцией
        {
          int dist = 0;
          std::string first_station, second_station;
          for (int i = 2; i < static_cast<int>(params.size());
               ++i) {  // Stop Tolstopaltsevo: 55.611087, 37.20829, 3900m to
                       // Marushkino, 4100m to Tret
            dist = stoi(std::string(params[i].begin(),
                                    params[i].begin() + params[i].find("m")));
            first_station = save_id[current_id];  // commain.id? why save_id
            second_station =
                std::string(params[i].begin() + params[i].find(" to ") + 4,
                            params[i].end());  // мб запятая
            catalogue.AddDistance(std::move(first_station),
                                  std::move(second_station), dist);
          }
        }
        current_id++;
      }
...
void inreader::InputReader::StartParcing(
    transport_catalogue::processing::TransportCatalogue& catalogue) {
