...
void TransportRouter::CreateWaitEdges(
  ...
  for (auto& [name, id] : stopname_id_data_) {
    graph.AddEdge(graph::Edge{id, stopname_id_data_.size() + id,
                              set_.wait_time_, graph::StatusEdge::WAIT, 0,
                              name});
  }
}
  ...
  for (auto& bus : ts.GetBusData()) {
    ...
    if (bus.is_roundtrip_) {
      for (auto& stop : bus.stops_) {
        ...
        for (int i = count; static_cast<size_t>(i) < bus.stops_.size();
             ++i)  // a b c b a    a b a c // b a b c /
        {
          prev_distance += static_cast<double>(ts.FindDist(
                               const_cast<domain::Stop*>(bus.stops_.at(i - 1)),
                               const_cast<domain::Stop*>(bus.stops_.at(i)))) /
                           METERS_IN_KM;
      ...
      count = 1;
      for (int j = 0; static_cast<size_t>(j) < bus.stops_.size() / 2;
           ++j)  // a b c b a
      {
        prev_distance = 0;
        span_count = 1;
        auto stop = bus.stops_.at(j);
        for (int i = count; static_cast<size_t>(i) < bus.stops_.size() / 2 + 1;
             ++i)  // a b
        {
          prev_distance += static_cast<double>(ts.FindDist(
                               const_cast<domain::Stop*>(bus.stops_.at(i - 1)),
      ...
      for (int j = bus.stops_.size() / 2;
           static_cast<size_t>(j) < bus.stops_.size(); ++j) {
        span_count = 1;
        prev_distance = 0;
        auto stop = bus.stops_.at(j);
        for (int i = bus.stops_.size() / 2 + count;
             static_cast<size_t>(i) < bus.stops_.size(); ++i)  // a b c b a
        {
          prev_distance += static_cast<double>(ts.FindDist(
                               const_cast<domain::Stop*>(bus.stops_.at(i - 1)),
                               const_cast<domain::Stop*>(bus.stops_.at(i)))) /
